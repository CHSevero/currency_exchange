...
def sample_transactions(db_session):
    ...
    base_time = datetime.now(timezone.utc).replace(microsecond=0)  # Remove microseconds for consistent comparison
    transactions = []
...
def test_get_user_transactions_with_pagination(transaction_service, db_session, sample_transactions):
...
def test_get_user_transactions_with_date_filter(transaction_service, db_session):
    ...
    all_txns = db_session.query(Transaction).order_by(Transaction.timestamp.desc()).all()
    ...
    assert result["total"] == 3, f"Expected total of 3 transactions, got {result['total']}"
    
    # Verify timestamps are correct
    for tx in result["transactions"]:
        tx_time = tx["timestamp"]  # Already a datetime object
        logger.info(f"Verifying transaction timestamp {tx_time} is >= from_date {from_date}")
        assert tx_time >= from_date, f"Transaction timestamp {tx_time} is before from_date {from_date}"
        assert (tx_time - from_date) <= timedelta(days=2), f"Transaction timestamp {tx_time} is more than 2 days after from_date"
...
def test_transaction_format(transaction_service, db_session, sample_transactions):
    ...
    assert all(key in transaction for key in ["transaction_id", "from", "to", "rate", "timestamp"])
