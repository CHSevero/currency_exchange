...
class RateService:
    ...
    async def _fetch_from_external_api(self) -> Dict[str, Decimal]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            decimal_rates = {currency: Decimal(str(rate)) for currency, rate in data["rates"].items()}
    ...
    def _get_rates_from_db(self, db: Session) -> Optional[Dict[str, Decimal]]:
        ...
        try:
            ...
            if exchange_rate:
                ...
                return {currency: Decimal(rate) for currency, rate in exchange_rate.rates.items()}
